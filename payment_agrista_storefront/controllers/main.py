...
from odoo.addons.payment_agrista import utils as agrista_utils
...
class AgristaControllerInherit(AgristaController):

    _webhook_url = AgristaController()._webhook_url

    @http.route('/payment/agrista/payments', type='json', auth='public')
    def agrista_payments(
        ...
        if tx_sudo.created_on_storefron:
        ...
        data = {
            'merchantAccount': provider_sudo.agrista_merchant_account,
            'amount': {
            ...
            'shopperReference': provider_sudo._agrista_compute_shopper_reference(partner_id),
            'recurringProcessingModel': 'CardOnFile',  # Most susceptible to trigger a 3DS check
            'shopperIP': shopper_ip,
            'shopperInteraction': 'Ecommerce',
            'shopperEmail': tx_sudo.partner_email,
            'shopperName': agrista_utils.format_partner_name(tx_sudo.partner_name),
            ...
            'returnUrl': urls.url_join(
                ...
                f'/payment/agrista/return?merchantReference={reference}'
            ),
            **agrista_utils.include_partner_addresses(tx_sudo),
        }
        ...
        # Make the payment request to Adyen
        response_content = provider_sudo._agrista_make_request(
            url_field_name='agrista_checkout_api_url',
            endpoint='/payments',
            payload=data,
            method='POST'
        )
        ...
        tx_sudo._handle_notification_data(
            'agrista', dict(response_content, merchantReference=reference),  # Match the transaction
        )
        return response_content

    @http.route('/payment/agrista/return', type='http', auth='public', csrf=False, save_session=False)
    def agrista_return_from_3ds_auth(self, **data):
        ...
        result = self.agrista_payment_details(
    ...
    def agrista_webhook(self):
        """ Process the data sent by Adyen to the webhook based on the event code.

        See https://docs.agrista.com/development-resources/webhooks/understand-notifications for the
        ...
        for notification_item in data['notificationItems']:
            ...
            try:
                ...
                tx_sudo._handle_notification_data('agrista', notification_data)
