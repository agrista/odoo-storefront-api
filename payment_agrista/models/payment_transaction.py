...
from odoo.addons.payment_agrista.models.agrista_request import AgristaAPI
from odoo.addons.payment_agrista.const import TRANSACTION_STATUS_MAPPING
...
class PaymentTransaction(models.Model):
    ...
    def _agrista_create_transaction_request(self, opaque_data):
        """ Create an Agrista payment transaction request.
        ...
        agrista_api = AgristaAPI(self.provider_id)
        if self.provider_id.capture_manually or self.operation == 'validation':
            return agrista_api.authorize(self, opaque_data=opaque_data)
        else:
            return agrista_api.auth_and_capture(self, opaque_data=opaque_data)
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id.agrista_profile:
            raise UserError("Agrista: " + _("The transaction is not linked to a token."))

        agrista_api = AgristaAPI(self.provider_id)
        if self.provider_id.capture_manually:
            res_content = agrista_api.authorize(self, token=self.token_id)
            _logger.info(
                "authorize request response for transaction with reference %s:\n%s",
                self.reference, pprint.pformat(res_content)
            )
        else:
            res_content = agrista_api.auth_and_capture(self, token=self.token_id)
            _logger.info(
                "auth_and_capture request response for transaction with reference %s:\n%s",
                self.reference, pprint.pformat(res_content)
            )
        self._handle_notification_data('agrista', {'response': res_content})
    ...
    def _send_refund_request(self, amount_to_refund=None):
        ...
        agrista_api = AgristaAPI(self.provider_id)
        tx_details = agrista_api.get_transaction_details(self.provider_reference)
        if 'err_code' in tx_details:  # Could not retrieve the transaction details.
            raise ValidationError("Agrista: " + _(
        ...
        if tx_status in TRANSACTION_STATUS_MAPPING['voided']:
            ...
            if tx_status in TRANSACTION_STATUS_MAPPING['authorized']:
                ...
                res_content = agrista_api.void(self.provider_reference)
                tx_to_process = self
            else:
                ...
                res_content = agrista_api.refund(
            ...
            tx_to_process._handle_notification_data('agrista', data)
        else:
    ...
    def _send_capture_request(self, amount_to_capture=None):
        ...
        res_content = agrista_api.capture(self.provider_reference, rounded_amount)
        _logger.info(
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if provider_code != 'agrista' or len(tx) == 1:
            return tx

        reference = notification_data.get('reference')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'agrista')])
        if not tx:
            raise ValidationError(
                "Agrista: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _agrista_tokenize(self):
        ...
        agrista_api = AgristaAPI(self.provider_id)
        cust_profile = agrista_api.create_customer_profile(
        ...
        if cust_profile:
            token = self.env['payment.token'].create({
                ...
                'agrista_profile': cust_profile.get('profile_id'),
                'agrista_payment_method_type': self.provider_id.agrista_payment_method_type,
                'verified': True,
            })
            self.write({
                'token_id': token.id,
