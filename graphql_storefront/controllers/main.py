...
class StorefrontBinary(Binary):
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        return super(StorefrontBinary, self).content_image(
...
class GraphQLController(http.Controller, GraphQLControllerMixin):
    ...
    def _process_request(self, schema, data):
        # Set the storefront_debug_mode value that exist in the settings
        ICP = http.request.env['ir.config_parameter'].sudo()
        storefront_debug_mode = ICP.get_param('storefront_debug_mode', False)
        if storefront_debug_mode:
            try:
    ...
    @http.route("/graphiql/storefront", auth="user")
    ...
    @http.route("/graphql/storefront", auth="public", csrf=False)
    def graphql(self, **kwargs):
        self._set_website_context()
        return self._handle_graphql_request(schema.graphql_schema)

    @http.route('/storefront/categories', type='http', auth='public', csrf=False)
    def storefront_categories(self):
        self._set_website_context()
    ...
    @http.route('/storefront/products', type='http', auth='public', csrf=False)
    def storefront_products(self):
        self._set_website_context()
        website = request.env['website'].get_current_website()
    ...
    @http.route('/storefront/redirects', type='http', auth='public', csrf=False)
    def storefront_redirects(self):
        redirects = []
