...
class WebsiteRewrite(models.Model):
    ...
    def _get_storefront_tags(self):
        tags = 'WR%s' % self.id
        return tags

    def _storefront_request_cache_invalidation(self):
        ICP = self.env['ir.config_parameter'].sudo()
        url = ICP.get_param('storefront_cache_invalidation_url', False)
        key = ICP.get_param('storefront_cache_invalidation_key', False)

        if url and key:
            try:
                for website_rewrite in self:
                    tags = website_rewrite._get_storefront_tags()
    ...
    def write(self, vals):
        ...
        self._storefront_request_cache_invalidation()
        return res
