...
from odoo.addons.payment_agrista.const import CURRENCY_DECIMALS
from odoo.addons.graphql_storefront.schemas.objects import PaymentProvider, PaymentTransaction
from odoo.addons.graphql_storefront.schemas.shop import Cart, CartData
from odoo.addons.website_sale.controllers.main import PaymentPortal
from odoo.addons.payment_adyen.controllers.main import AdyenController
from odoo.addons.payment_agrista.controllers.main import AgristaController
...
class PaymentQuery(graphene.ObjectType):
    ...
    def resolve_payment_confirmation(self, info):
        ...
        # Pass in the session the sale_order created in storefront
...
class AdyenPaymentMethodsResult(graphene.ObjectType):
    agrista_payment_methods = generic.GenericScalar()
...
class AdyenPaymentsResult(graphene.ObjectType):
    agrista_payments = generic.GenericScalar()


class AdyenPaymentDetailsResult(graphene.ObjectType):
    agrista_payment_details = generic.GenericScalar()
...
class AdyenPaymentMethods(graphene.Mutation):
    ...
    def mutate(self, info, provider_id):
        ...
        agrista_payment_methods = AdyenController().agrista_payment_methods(
            provider_id=payment_provider_id.id,
        ...
        return AdyenPaymentMethodsResult(agrista_payment_methods=agrista_payment_methods)
...
class AdyenTransaction(graphene.Mutation):
    ...
    def mutate(self, info, provider_id):
        ...
        # Update the field created_on_storefront
        transaction_id.created_on_storefront = True
...
class AdyenPayments(graphene.Mutation):
    ...
    def mutate(self, info, provider_id, transaction_reference, access_token, payment_method, browser_info):
        ...
        if not payment_provider_id.code == 'agrista':
            raise GraphQLError(_('Payment Provider "Agrista" does not exist.'))
        ...
        # Create Payment
        agrista_payment = AgristaController().agrista_payments(
            provider_id=payment_provider_id.id,
        ...
        return AdyenPaymentsResult(agrista_payments=agrista_payment)
...
class AdyenPaymentDetails(graphene.Mutation):
    ...
    def mutate(self, info, provider_id, transaction_reference, payment_details):
        ...
        # Submit the details
        agrista_payment_details = AdyenController().agrista_payment_details(
            provider_id=payment_provider_id.id,
            reference=transaction.reference,
            payment_details=payment_details
        )

        return AdyenPaymentDetailsResult(agrista_payment_details=agrista_payment_details)
...
class AdyenPaymentMutation(graphene.ObjectType):
    ...
    agrista_payment_methods = AdyenPaymentMethods.Field(description='Get Adyen Payment Methods.')
    adyen_transaction = AdyenTransaction.Field(description='Create Adyen Transaction')
    agrista_payments = AdyenPayments.Field(description='Make Adyen Payment request.')
    agrista_payment_details = AdyenPaymentDetails.Field(description='Submit the Adyen Payment Details.')
